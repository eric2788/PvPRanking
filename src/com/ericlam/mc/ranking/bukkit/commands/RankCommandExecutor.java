...
public class RankCommandExecutor implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ComponentBuilder author = new ComponentBuilder("§7插件作者: §m§bEric Lam").event(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://github.com/eric2788")).event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.fromLegacyText("§e打開 Github 連結")));
        ComponentBuilder source = new ComponentBuilder("§7>> 點這裏查看插件原始碼").event(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://github.com/PvPRanking"));
        if (strings.length < 2) {
            ...
            if (!commandSender.hasPermission("pvprank.admin")) return false;
        ...
        switch (method) {
            case "info":
                Bukkit.getScheduler().runTaskAsynchronously(pl, () -> {
                    ...
                    list.forEach(line -> {
                        String replaced = line.replace("<rank>", data.getRank()).replace("<score>", data.getFinalScores() + "")
            ...
            case "reset":
                Bukkit.getScheduler().runTaskAsynchronously(pl, () -> {
                    ...
                    commandSender.sendMessage("§c刪除 " + (success ? "成功。" : "失敗，可能查無資料。"));
                });
                return true;
            default:
